...
async function load() {
  try {
    ...
    const escolas = Array.from(
      new Set(allRows.map((r) => (r.escola || "").trim()).filter(Boolean)),
    ...
    $("lblAtualizacao").textContent =
      `Atualizado em ${fmtDateTime(new Date().toISOString())}`;
...
function applyFilters() {
  ...
  if (busca) {
    rows = rows.filter((r) => {
      const bag = [r.nome, r.protocolo, r.topico, r.observacoes, r.escola]
        .filter(Boolean)
        .join(" ")
        .toLowerCase();
...
function renderTable(rows) {
  ...
  rows.forEach((r) => {
    ...
    if (temaKey === "LICENCA_PREMIUM" || temaKey === "LICENCA_PREMIO") {
      const tFormatado = r.topico ? prettyTopico(r.topico) : "";
      stCell = tFormatado
        ? `<span class="badge-status ${statusClass(tFormatado)}">${escapeHtml(tFormatado)}</span>`
        : `<span class="muted">—</span>`;
    } else {
      stCell = r.status
        ? `<span class="badge-status ${statusClass(r.status)}">${escapeHtml(r.status)}</span>`
        : `<span class="muted">—</span>`;
    }

    const saidaDisplay =
      temaKey === "CONTAGEM_TEMPO"
        ? `<span class="muted">—</span>`
        : r.data_saida
...
function prettyTopico(t) {
  ...
  if (s.includes("devolvido")) return "Devolvido para Correção";
  return t;
}
